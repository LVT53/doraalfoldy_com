...
<head>
    ...
    <title>{{ __('emails.appointment_reminder_subject', ['date' => $appointment->start_time->format('Y.m.d.'), 'time' => $appointment->start_time->format('H:i')]) }}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <h1 style="color: #d4af37;">{{ __('emails.booking_confirmation_greeting', ['name' => $appointment->user_name]) }}</h1>
    
    <p>{{ __('emails.appointment_reminder_intro', ['hours' => $hoursUntil]) }}</p>
    ...
    <div style="background: #f9f9f9; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Helyszín:</strong> Dóra Alfoldy Photography Studio</p>
    </div>
