<nav class="navbar sticky top-0 z-[10000] backdrop-blur-[20px] bg-white/80 border-b border-brand-gold/5">
    <div class="w-[87%] mx-auto py-3 flex justify-between items-center text-neutral-600">
        ...
        <div class="lg:hidden" x-data="{ mobileMenuOpen: false }">
             ...
             <div x-show="mobileMenuOpen" 
                  ...
                  x-transition:enter-end="opacity-100 translate-y-0"
                  class="fixed inset-x-0 top-[80px] h-[calc(100vh-80px)] bg-white/95 backdrop-blur-2xl shadow-2xl py-12 px-8 flex flex-col space-y-8 z-[9999] overflow-y-auto">
                <a href="{{ route('home') }}" class="text-3xl font-serif font-bold {{ request()->routeIs('home') ? 'text-brand-gold' : 'text-neutral-900' }}" @click="mobileMenuOpen = false">Kezdőlap</a>
                
                <div x-data="{ open: true }">
                    <button @click="open = !open" class="text-3xl font-serif font-bold text-neutral-900 flex justify-between w-full items-center">
                        Szolgáltatások
                        <svg class="h-8 w-8 transition-transform duration-300" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" class="pl-6 mt-6 flex flex-col space-y-6 border-l-4 border-brand-gold/20">
                        <a href="{{ route('szempilla') }}" class="text-2xl font-medium text-neutral-600 hover:text-brand-gold" @click="mobileMenuOpen = false">Szempilla</a>
                        <a href="{{ route('smink') }}" class="text-2xl font-medium text-neutral-600 hover:text-brand-gold" @click="mobileMenuOpen = false">Smink</a>
                        <a href="{{ route('szemoldok') }}" class="text-2xl font-medium text-neutral-600 hover:text-brand-gold" @click="mobileMenuOpen = false">Szemöldök</a>
                        <a href="{{ route('smink-tanacsadas') }}" class="text-2xl font-medium text-neutral-600 hover:text-brand-gold" @click="mobileMenuOpen = false">Smink tanácsadás</a>
                    </div>
                </div>

                <a href="{{ route('galeria') }}" class="text-3xl font-serif font-bold {{ request()->routeIs('galeria*') ? 'text-brand-gold' : 'text-neutral-900' }}" @click="mobileMenuOpen = false">Galéria</a>
                ...
                <div x-data="{ open: false }">
                    ...
                    <div x-show="open" class="pl-6 mt-6 flex flex-col space-y-6 border-l-4 border-brand-gold/20">
                        <a href="{{ route('gdpr') }}" class="text-2xl font-medium text-neutral-600" @click="mobileMenuOpen = false">GDPR</a>
                        <a href="{{ route('aszf') }}" class="text-2xl font-medium text-neutral-600" @click="mobileMenuOpen = false">ÁSZF</a>
                    </div>
                </div>

                <div class="pt-8 text-center">
                    <a href="https://doraalfoldy.salonic.hu/" class="inline-block bg-brand-gold text-white px-10 py-5 rounded-full font-bold text-xl shadow-xl active:scale-95 transition-all">Foglalj időpontot!</a>
                </div>
             </div>
        </div>
    </div>
